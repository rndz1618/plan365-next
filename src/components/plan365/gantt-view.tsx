...
const TYPE_BG_COLORS: Record<string, string> = {
  '2D CAD': 'bg-violet-400', 'CAD': 'bg-sky-400', 'CAM': 'bg-emerald-400',
  'Tools': 'bg-orange-400', 'Others': 'bg-slate-400',
}
const TYPE_DARK_COLORS: Record<string, string> = {
  '2D CAD': 'bg-violet-700', 'CAD': 'bg-sky-700', 'CAM': 'bg-emerald-700',
  'Tools': 'bg-orange-700', 'Others': 'bg-slate-600',
}
...
export function GanttView({ projectId: propProjectId, embedded = false }: GanttViewProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const depUrl = projectId ? `/api/dependencies?projectId=${projectId}` : `/api/dependencies`
      const cpUrl = projectId ? `/api/critical-path?projectId=${projectId}` : null
  ...
  const { timelineStart, totalDays, dates } = useMemo(() => {
    ...
    if (selectedProject?.dueDate) maxDate = parseISO(selectedProject.dueDate)
    ...
    if (zoom === 'day') {
      ...
      start = minDate ? addDays(startOfDay(minDate), -3) : weekBeforeToday
      end = maxDate ? addDays(startOfDay(maxDate), 7) : addDays(today, 30)
    }

    const days = Math.max(1, differenceInDays(end, start) + 1)
    const d = Array.from({ length: days }, (_, i) => addDays(start, i))
    return { timelineStart: start, totalDays: days, dates: d }
  }, [tasks, selectedProject, zoom])
  ...
  const getTaskX = useCallback((task: Task) => {
    ...
    return differenceInDays(taskStart, timelineStart) * colWidth
  ...
  const todayColIndex = useMemo(() => differenceInDays(startOfDay(new Date()), timelineStart), [timelineStart])
  ...
  const headerLabels = useMemo(() => {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      const w = format(d, 'w')
      if (zoom === 'day') {
        if (m !== lastMonth) { labels.push({ text: m, x: i * colWidth, isMonth: true }); lastMonth = m }
        labels.push({ text: config.labelFn(d), x: i * colWidth, isWeekend: isWeekend(d), isToday: isToday(d) })
      } else if (zoom === 'week') {
        if (w !== lastWeek) {
          labels.push({ text: format(d, 'd MMM'), x: i * colWidth, isWeekend: isWeekend(d), isToday: isToday(d) })
          lastWeek = w
        }
      } else if (m !== lastMonth) {
        labels.push({ text: m, x: i * colWidth, isMonth: true }); lastMonth = m
      }
    }
    return labels
  }, [dates, zoom, colWidth])
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b bg-card shrink-0 gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          {(['month', 'week', 'day'] as ZoomLevel[]).map((z) => (
            <Button key={z} variant={zoom === z ? 'default' : 'outline'} size="sm" onClick={() => setZoom(z)}
            ...
            <div className="h-full flex flex-col border-r">
              <div className="shrink-0 bg-muted/50 border-b px-2 py-1.5" style={{ height: HEADER_HEIGHT }}>
                <div className="grid grid-cols-[40px_1fr_80px_70px_50px] gap-1 text-[11px] font-medium text-muted-foreground uppercase tracking-wide">
                  <span>ID</span><span>Title</span><span>Assignee</span><span>Start</span><span>Effort</span>
                </div>
              </div>
              <div ref={leftScrollRef} onScroll={onLeftScroll} className="flex-1 overflow-auto">
                {tasks.map((t) => {
                  ...
                  return (
                    <div key={t.id} className={cn('grid grid-cols-[40px_1fr_80px_70px_50px] gap-1 items-center px-2 border-b text-xs', isCritical && 'bg-red-50 dark:bg-red-950/20')} style={{ height: ROW_HEIGHT }}>
                      ...
                      <span>{t.assignee && <UserAvatar user={t.assignee} size="sm" />}</span>
                      <span className="text-muted-foreground">{t.startDate ? format(parseISO(t.startDate), 'MMM d') : '—'}</span>
                      <span className="text-muted-foreground">{t.effort != null ? `${t.effort}h` : '—'}</span>
                    </div>
                  )
                })}
              </div>
            ...
            <div className="h-full flex flex-col">
                ...
                <div style={{ width: totalWidth }}>
                  <div className="relative h-[22px]">
                    {headerLabels.filter((l) => l.isMonth).map((l, i) => (
                      <span key={i} className="absolute text-[10px] font-semibold text-muted-foreground top-1" style={{ left: l.x + 4 }}>{l.text}</span>
                    ))}
                  </div>
                  <div className="relative h-[32px] flex">
                    {headerLabels.filter((l) => !l.isMonth).map((l, i) => (
                      <span key={i} className={cn('absolute text-[10px] flex items-center justify-center top-0', l.isToday && 'font-bold text-emerald-600', l.isWeekend && 'text-muted-foreground/40', !l.isToday && !l.isWeekend && 'text-muted-foreground')}
                        style={{ left: l.x, width: zoom === 'week' ? colWidth * 7 : colWidth }}>{l.text}</span>
                    ))}
                  </div>
                </div>
              </div>
              <div ref={rightScrollRef} onScroll={onRightScroll} className="flex-1 overflow-auto">
                <div className="relative" style={{ width: totalWidth, minHeight: tasks.length * ROW_HEIGHT }}>
                  {zoom === 'day' && dates.map((d, i) => isWeekend(d) ? (
                  ...
                  {tasks.map((t, idx) => {
                    ...
                    return (
                      <div key={t.id} className={cn('absolute group', isCritical && 'drop-shadow-[0_0_3px_rgba(239,68,68,0.5)]')}
                        style={{ top: barY, left: x, width: t.isMilestone ? MILESTONE_SIZE : w, height: 20, zIndex: 5 }}
                        title={`${t.title} (${t.type})\n${t.startDate || 'No start'} → ${t.dueDate || 'No due'}\nProgress: ${progress}%`}>
                        {t.isMilestone ? (
                          <div className={cn('w-full h-full flex items-center justify-center bg-amber-400', isCritical && 'ring-2 ring-red-500')}
                          ...
                          <div className={cn('relative w-full h-full rounded-sm overflow-hidden', TYPE_BG_COLORS[t.type] || 'bg-slate-400', isCritical && 'ring-2 ring-red-500 ring-offset-1')}>
                            ...
                            {w > 80 && <span className="absolute inset-0 flex items-center px-1.5 text-[10px] font-medium text-white truncate">{t.title}</span>}
                          </div>
                        )}
                      </div>
                    )
