...
export function TasksView() {
  ...
  useEffect(() => {
    if (newDialogOpen) fetchProjects()
  ...
  const columns = useMemo<ColumnDef<Task>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-xs text-muted-foreground">{row.original.effort ? `${row.original.effort}h` : '—'}</span>
        ),
        size: 70,
      },
      {
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a href={csvUrl} download>
            <Button variant="outline" size="sm"><Download className="h-4 w-4 mr-1.5" />CSV</Button>
          </a>
                ...
                {templateTasks.map((tt, idx) => (
                  <div key={idx} className="grid grid-cols-[1fr_80px_120px_120px] gap-2 items-end rounded-lg border p-3">
                    <div>
                      ...
                      <Input value={tt.title} onChange={(e) => updateTemplateTask(idx, 'title', e.target.value)} className="h-8 mt-1" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Effort (h)</Label>
                      <Input type="number" value={tt.effort ?? ''} onChange={(e) => updateTemplateTask(idx, 'effort', e.target.value ? Number(e.target.value) : null)} className="h-8 mt-1" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Start</Label>
                      <Input type="date" value={tt.startDate} onChange={(e) => updateTemplateTask(idx, 'startDate', e.target.value)} className="h-8 mt-1" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Due</Label>
                      <Input type="date" value={tt.dueDate} onChange={(e) => updateTemplateTask(idx, 'dueDate', e.target.value)} className="h-8 mt-1" />
                    </div>
                  </div>
                ))}
              </div>
      ...
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          <div className="grid gap-1.5">
            <Label>Project *</Label>
            <Select
              value={form.projectId ? String(form.projectId) : selectedProjectId ? String(selectedProjectId) : ''}
