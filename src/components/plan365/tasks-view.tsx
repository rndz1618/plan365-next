...
import { loadTeamUsers } from '@/lib/team-users'
import { cachedGet, invalidateAfterMutation } from '@/lib/api-cache'
...
export function TasksView() {
  ...
  const active = currentView === 'tasks'
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const { ok, data } = await cachedGet<{ tasks?: Task[] } | Task[]>(`/api/tasks?${params}`, { ttlMs: 12_000 })
      if (ok && data) {
  ...
  useEffect(() => { if (active) fetchTasks() }, [fetchTasks, active])
  useEffect(() => { if (active) void loadTeamUsers() }, [active])
  useEffect(() => { if (active) fetchProjects() }, [fetchProjects, active])
  ...
  const cycleStatus = async (task: Task, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.ok) { invalidateAfterMutation('tasks'); fetchTasks() }
    } catch { /* ignore */ }
  }
  ...
  const createFromTemplate = async () => {
    ...
    try {
      ...
      if (res.ok) { setTemplateDialogOpen(false); invalidateAfterMutation('tasks'); fetchTasks() }
    } catch { /* ignore */ }
    setSaving(false)
  }
