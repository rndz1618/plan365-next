...
export function TasksView() {
  ...
  const handleNewTask = async () => {
    ...
    try {
      const body = {
        ...form,
        projectId: selectedProjectId || form.projectId || 0,
  ...
  // ── Filtered tasks for column filter model ──
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2 min-w-[200px]">
          ...
          {row.original.isMilestone && (
            <Badge variant="outline" className="shrink-0 border-amber-400 text-amber-600 bg-amber-50 dark:bg-amber-950/30">
              ◆ Milestone
            </Badge>
          )}
        </div>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs text-muted-foreground">
          {row.original.effort ? `${row.original.effort}h` : '—'}
        </span>
      ),
      size: 70,
    },
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              onClick={(e) => confirmDelete(row.original, e)}
            >
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
        ...
        <TabsContent value="list" className="flex-1 min-h-0 overflow-y-auto">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{table.getFilteredSelectedRowModel().rows.length} of {table.getFilteredRowModel().rows.length} selected</span>
            </div>
      ...
      <TaskFormDialog
        open={editDialogOpen}
        onOpenChange={(v) => { setEditDialogOpen(v); if (!v) setEditTask(null) }}
        form={form}
        setForm={setForm}
        users={users}
        projects={projects}
        ...
        title="Edit Task"
        description={`Editing: ${editTask?.title ?? ''}`}
      />
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={doDelete} className="bg-red-600 hover:bg-red-700 focus-visible:ring-red-600">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <Dialog open={templateDialogOpen} onOpenChange={setTemplateDialogOpen}>
        <DialogContent className="sm:max-w-2xl max-h-[80vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              {templateStep === 'list'
                ? 'Select a template to create tasks from.'
                : `Edit tasks from "${selectedTemplate?.name}" before creating.`}
            </DialogDescription>
          </DialogHeader>
                  ...
                  {templates.map((tmpl) => (
                    ...
                    >
                      <div className="rounded-lg bg-emerald-100 dark:bg-emerald-900/30 p-2 shrink-0">
                        <BookTemplate className="h-5 w-5 text-emerald-600" />
                      </div>
                  ...
                  <div key={idx} className="grid grid-cols-[1fr_80px_120px_120px] gap-2 items-end rounded-lg border p-3">
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={tt.effort ?? ''}
                        onChange={(e) => updateTemplateTask(idx, 'effort', e.target.value ? Number(e.target.value) : null)}
                        className="h-8 mt-1"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="date"
                        value={tt.startDate}
                        onChange={(e) => updateTemplateTask(idx, 'startDate', e.target.value)}
                        className="h-8 mt-1"
                      />
                    </div>
                    <div>
          ...
          <DialogFooter>
            {templateStep === 'edit' && (
              <Button variant="outline" onClick={() => setTemplateStep('list')}>Back</Button>
            )}
...
// ── Task Form Dialog (shared for New & Edit) ──
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          {/* Project (only if creating without global project filter) */}
          {!selectedProjectId && (
            <div className="grid gap-1.5">
              <Label>Project</Label>
              <Select value={String(form.projectId || '')} onValueChange={(v) => update('projectId', Number(v))}>
