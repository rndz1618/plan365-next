...
function ProjectForm({
  ...
  if (selectedTemplate) {
    try {
      const parsed = JSON.parse(selectedTemplate.tasksJson || '[]')
      taskCount = Array.isArray(parsed) ? parsed.filter((r: { title?: string }) => r?.title?.trim()).length : 0
    } catch {
      taskCount = 0
    }
  }
        ...
        <div className="grid gap-2">
          ...
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder="None — empty project" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__none__">None — empty project</SelectItem>
              {templates.map((t) => (
          ...
          {selectedTemplate && (
            <p className="text-xs text-muted-foreground">
              Will create <span className="font-medium text-foreground">{taskCount}</span> task{taskCount !== 1 ? 's' : ''} from “{selectedTemplate.name}”.
    ...
    try {
      ...
      const res = await fetch('/api/projects', {
        ...
        body: JSON.stringify({
          ...
          templateId: newForm.templateId === '__none__' ? null : newForm.templateId,
        }),
      })
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-3 pb-4 border-b shrink-0">
          <Button variant="ghost" size="icon" className="h-9 w-9 shrink-0" onClick={() => setSelectedProject(null)}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <span className="h-3 w-3 rounded-full shrink-0" style={{ backgroundColor: selectedProject.color }} />
          <h2 className="text-lg font-semibold truncate min-w-0">{selectedProject.name}</h2>
          ...
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-red-600 shrink-0" onClick={() => openEdit(selectedProject)}>
            <Pencil className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-red-600 shrink-0" onClick={() => openDelete(selectedProject)}>
        ...
        <Dialog open={newOpen} onOpenChange={setNewOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Create a new project. Optionally apply a task template.</DialogDescription>
            </DialogHeader>
            <ProjectForm form={newForm} setForm={setNewForm} isEdit={false} templates={templates} />
            <DialogFooter>
        ...
        <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={(e) => { e.preventDefault(); handleDelete() }} disabled={deleting} className="bg-red-600 hover:bg-red-700 focus:ring-red-600">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" asChild>
            <a href="/api/export?type=projects"><Download className="mr-1.5 h-4 w-4" />Export CSV</a>
          </Button>
      ...
      {filteredProjects.length === 0 ? (
        <EmptyState message={search ? 'No projects match your search' : 'No projects yet. Create your first project!'} icon={FolderOpen} />
      ) : (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project) => {
            const pct = getCompletionPercent(project)
            return (
              <Card key={project.id} className={cn('cursor-pointer overflow-hidden transition-shadow hover:shadow-md border-l-4')} style={{ borderLeftColor: project.color }} onClick={() => setSelectedProject(project)}>
                <CardContent className="space-y-4 p-5">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <h3 className="truncate font-semibold">{project.name}</h3>
                      {project.reference && <p className="mt-0.5 text-xs text-muted-foreground">{project.reference}</p>}
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    {project.dueDate && (<><span className="mx-1">—</span>{format(parseISO(project.dueDate), 'MMM d, yyyy')}</>)}
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><ListChecks className="h-3.5 w-3.5" />{project._count?.tasks || 0} tasks</span>
                      <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{project._count?.members || 0} members</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      <div className="flex -space-x-2">
                        {project.members.slice(0, 3).map((m) => (
                          <span key={m.id} className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-600 text-[10px] font-semibold text-white ring-2 ring-background" title={m.fullName || m.username}>
                            {m.fullName ? (m.fullName.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()) : m.username.slice(0, 2).toUpperCase()}
                          </span>
                        ))}
                        {project.members.length > 3 && (
                          <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-muted text-[10px] font-medium ring-2 ring-background">+{project.members.length - 3}</span>
                        )}
                      </div>
                    </div>
                  )}
                  <div className="flex items-center justify-end gap-1 border-t pt-3">
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); openEdit(project) }}>
                      <Pencil className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-red-600" onClick={(e) => { e.stopPropagation(); openDelete(project) }}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                </CardContent>
